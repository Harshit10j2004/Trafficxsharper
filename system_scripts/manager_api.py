...
def get_all_node_ids():
    output = subprocess.check_output(
        ["sudo","docker", "node", "ls", "--format", "{{.ID}} {{.Status}}"]
    ).decode()
...
def get_node_load(node_id: str):
    try:
        ...
        stats = subprocess.check_output(
            ["sudo","docker", "stats", "--no-stream", "--format", "{{.CPUPerc}} {{.MemPerc}}"]
        ).decode().strip().split()
        if len(stats) >= 2:
    ...
    while time.time() - start < timeout:
        try:
            ...
            result = subprocess.run(
                ["sudo","docker", "node", "ps", node_id, "--format", "{{.ID}}"],
...
def removing_node(data:Data):
    ...
    node_status = subprocess.check_output(["sudo","docker", "node", "inspect", node_id, "--format", "{{.Status.State}}"]).decode().strip()
    ...
    subprocess.run(["sudo","docker", "node", "update", "--availability", "drain", node_id], check=True)
